...
//Yada yada, just dumping all the crop values into the text file.  For good measure, I'm throwing in the intensity value of the fish, too.
	...
	char cleft[50], cright[50], ctop[50], cbottom[50];
	//char cfish_color[50];
	...
	//string dir_beta = str;
	//dir_beta.erase(4); //changed it for quick analysis w/ run.sh, crop, and imager all in same directory as the .wmv files.

	FILE * pFile;
	/*string dir_beta = "crop.txt";
	char cropfile[(int)dir_beta.length()];
	for (int i=0;i<(int)dir_beta.length();i++){
		cropfile[i]=dir_beta[i];
	}
	cropfile[(int)str.length()]='\0';*/ //just gonna call it "crop.txt"...

	pFile = fopen("crop.txt", "a");
	fputs (cleft,pFile);
	fputs ("\n",pFile);
	fputs (cright,pFile);
	fputs ("\n",pFile);
