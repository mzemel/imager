...
int orientation (CImg<unsigned char> &image, float &angle, int &validity){
	int x = image.width();
	int y = image.height();
			...
			if (str.at(5) == '0'){
				if (str.at(6) == '0'){
					if (str.at(7) == '0'){
						...
						str2.insert(0,"/");
						str2.insert(0,dir);
						const char * temp_image_name = str2.c_str();
						str.append(".jpg");
						str.insert(0,"/");
						str.insert(0,dir);
						char r[(int)str.length()];
						...
						r[(int)str.length()]='\0';
						temp_image.load(r);
						file_for_pixel.open(temp_image_name);
						for (int n=0;n<(int)temp_image.height();n++){
							for (int m=0;m<(int)temp_image.width();m++){
								file_for_pixel << temp_image.atXY(m,n) << "\n";
							}
						}
